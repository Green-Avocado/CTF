...
We're given 2 RSA encrypted cyphertexts, along with their respective moduli and exponents.

The moduli are the same, the exponents and cyphertexts are different.

## Solution

Assuming the original message is the same, this is vulnerable to the common modulus attack, allowing us to recover the original message.

I used [lucif3r's script](https://blog.0daylabs.com/2015/01/17/rsa-common-modulus-attack-extended-euclidean-algorithm/) to solve this.
...
class RSAModuli:
    ...
    def print_value(self):
        hex_string = hex(self.m)
        flag = bytes.fromhex(hex(self.m)[2:]).decode("ascii")
        print("Plain Text: ", flag)
